...
### ✨ 新機能
- **Issue #48 – TCPサーバー**: 仮想Zebraプリンターモードを実装。`ZPL2PDF server start --port 9101 -o output/`、`server stop`、`server status`を使用。
- **REST API (PR #47)**: `ZPL2PDF --api --host localhost --port 5000`で`POST /api/convert`（ZPLをPDFまたはPNGに）と`GET /api/health`を提供。[APIガイド](../guides/API_GUIDE.md)。
...
- **Issue #39**: 同じ名前の複数グラフィックの順次処理
  - 複数の`~DGR`グラフィックを含むZPLファイルが正しく処理されるようになりました
  - 各ラベルは順次状態に基づいて正しいグラフィックを使用
  - `^IDR`クリーンアップコマンドで空白ページが生成されなくなりました
  - Shopee配送ラベルファイルで全ラベルが同一だった問題を解消

### 🔧 改善
- 公開メソッドへの入力検証の追加
- 例外処理の改善
- コンパイル済み正規表現によるパフォーマンス最適化
- コードのクリーンアップと未使用メソッドの削除
